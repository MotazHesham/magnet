    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.coupons.update", [$coupon->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $coupon->discount) }}" step="0.01" required max="100">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.coupon.fields.discount_type') }}</label>
            ...
            <div class="form-group">
                <label for="stores">{{ trans('cruds.coupon.fields.stores') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('stores') ? 'is-invalid' : '' }}" name="stores[]" id="stores" multiple>
                    @foreach($stores as $id => $store)
                        <option value="{{ $id }}" {{ (in_array($id, old('stores', [])) || $coupon->stores->contains($id)) ? 'selected' : '' }}>{{ $store }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.coupon.fields.stores_helper') }}</span>
            </div>
            <div class="form-group">
