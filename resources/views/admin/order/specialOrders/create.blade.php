...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.specialOrder.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.special-orders.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('variations') ? 'is-invalid' : '' }}" name="variations" id="variations" required>{{ old('variations') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('delivery_status') ? 'is-invalid' : '' }}" name="delivery_status" id="delivery_status" required>
                    ...
                    @foreach(App\Models\SpecialOrder::DELIVERY_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('delivery_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('offer_price_status') ? 'is-invalid' : '' }}" name="offer_price_status" id="offer_price_status" required>
                    ...
                    @foreach(App\Models\SpecialOrder::OFFER_PRICE_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('offer_price_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('payment_data') ? 'is-invalid' : '' }}" name="payment_data" id="payment_data">{{ old('payment_data') }}</textarea>
