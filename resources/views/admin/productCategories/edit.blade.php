                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($parents as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('parent_id') ? old('parent_id') : $productCategory->parent->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        <label class="required"
                            for="order_level">{{ trans('cruds.productCategory.fields.order_level') }}</label>
                        <input class="form-control {{ $errors->has('order_level') ? 'is-invalid' : '' }}" type="number"
                            name="order_level" id="order_level"
                            value="{{ old('order_level', $productCategory->order_level) }}" step="1" required>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <div class="needsclick dropzone {{ $errors->has('banner') ? 'is-invalid' : '' }}"
                            id="banner-dropzone">
                        </div>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <textarea class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}" name="meta_description"
                            id="meta_description">{{ old('meta_description', $productCategory->meta_description) }}</textarea>
