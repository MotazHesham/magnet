...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.customerScratch.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-scratches.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('scratch') ? 'is-invalid' : '' }}" name="scratch_id" id="scratch_id" required>
                    @foreach($scratches as $id => $entry)
                        <option value="{{ $id }}" {{ old('scratch_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expire_at') ? 'is-invalid' : '' }}" type="text" name="expire_at" id="expire_at" value="{{ old('expire_at') }}" required>
