...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.refundRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.refund-requests.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('special_order') ? 'is-invalid' : '' }}" name="special_order_id" id="special_order_id">
                    @foreach($special_orders as $id => $entry)
                        <option value="{{ $id }}" {{ old('special_order_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('store_approval') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="store_approval" id="store_approval" value="1" {{ old('store_approval', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('admin_approval') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="admin_approval" id="admin_approval" value="1" {{ old('admin_approval', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('refund_status') ? 'is-invalid' : '' }}" name="refund_status" id="refund_status">
                    ...
                    @foreach(App\Models\RefundRequest::REFUND_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('refund_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
