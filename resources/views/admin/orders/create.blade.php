        ...
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('combined_order') ? 'is-invalid' : '' }}" name="combined_order_id" id="combined_order_id">
                    @foreach($combined_orders as $id => $entry)
                        <option value="{{ $id }}" {{ old('combined_order_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('store_approval') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="store_approval" id="store_approval" value="1" required {{ old('store_approval', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_status') ? 'is-invalid' : '' }}" name="payment_status" id="payment_status" required>
                    ...
                    @foreach(App\Models\Order::PAYMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_status', 'un_paid') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('shipping_type') ? 'is-invalid' : '' }}" name="shipping_type" id="shipping_type">
                    ...
                    @foreach(App\Models\Order::SHIPPING_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('shipping_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
